...
export default {
  ...
  methods: {
    async initial_data() {
      ...
      await axios
        .post(
          this.$store.state.common.baseUrl + "/api/disposisi/show_id",
          { id: id },
          {
            headers: {
        ...
        .then((res) => {
          this.disposisi_baru = res.data.data;
        })
        .catch(() => {
    ...
    async kirim_data() {
      if (this.disposisi_baru.irban == undefined) {
        ...
        await axios
          .post(
            this.$store.state.common.baseUrl + "/api/disposisi/edit",
            {
          ...
          .catch(() => {
            ...
            alert.show = true;
            alert.type = "error";
            alert.message = "Maaf, Gagal edit disposisi";
            window.setTimeout(() => {
              alert.show = false;
            }, 3000);
          });
